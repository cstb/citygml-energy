...
While separating two thermal zones, its optional geometry corresponds to the middle of the internal/shared wall.
While separating a thermal zone from the building surrounding, its optional geometry corresponds to the external surface of the outer wall/roof/basement floor. The following figure represents these different cases in a building side section, relating the Energy ADE Objects `ThermalZone` and `ThermalBoundary` to the CityGML objects `Room` (could be also `Building` or `BuildingPart`) respectively `WallSurface`, `RoofSurface`, `FloorSurface`, `CeilingSurface` and `InteriorWallSurface`.
